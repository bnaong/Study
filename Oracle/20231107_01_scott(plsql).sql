...
-- 변수를 선언할 때 메모리가 할당(소모)되는데..
-- 프로시저의 경우 생성된 프로시저를 실행할지 안할지에 대해 사용자가 선택가능하지만..
-- 트리거는 사용자의 의지대로 실행될지 말지 선택가능한 것이 아니라
-- 예를 들어 INSERT가 실행되면 TRIGGER가 작동하는 등
-- 사용자의 의지와 상관없이 실행되기 때문에
-- 변수를 선언하여 메모리소모(비용)가 많이 발생하면 안될 것이다.
-- 변수를 절대 선언해서는 안되는다는 것은 아니지만, 메모리가 부족해서 트리거가 발생되지 않을 수 있으므로
-- 메모리소모(비용)을 낮추어야 한다..
...
BEGIN
    ...
    ELSIF (UPDATING)
        THEN UPDATE TBL_상품
            ...
            WHERE 상품코드 = :OLD.상품코드;     -- :NEW 이든 :OLD 이든 상관 X
    ELSIF (DELETING)
        THEN UPDATE TBL_상품
