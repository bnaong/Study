...
<%
	...
	String searchKey = request.getParameter("searchKey");		// 제목(subject, key)
	String searchValue = request.getParameter("searchValue");	// 날씨(		value)
	...
	if (searchKey != null)	//-- 검색 기능을 통해 이 페이지가 요청되었을 경우
	{
		...
		// 넘어온 요청이 GET방식이라면... 디코딩 처리할 수 있도록 코드 구성
	...
	//-- 현재 페이지가 1인 경우(currentPage=1) 1 → 1 ~ 10
	//   데이터베이스에서 가져올 게시물은 1번째(start : 1) 부터 10번째(end : 10) 까지
	//   현제 페이지가 2인 경우(currentPage=2)
	//   데이터베이스에서 가져올 게시물은 11번째(start : 11) 부터 20번째(end : 20) 까지
	
	// 가져올 리스트의 대역폭 확정~!!!
	
	// 실제 리스트 가져오기 → 검색 데이터 리스트 구하기로 변경 예정~!!!
	List<BoardDTO> lists = dao.getLists(searchKey, searchValue, start, end);
	...
	//-- article 페이지에서는 pageNum 이 필요 없지만, 목록으로 돌아가거나 할 경우에
	//   원래의 리스트 페이지로 돌아가게끔 하려면 pageNum 이 필요함
	...
	<div id="bbsList_header">
		<div id="leftHeader">
			...
			<!-- <form action="" name="searchForm" method="get"> -->
				...
				<!-- 위에서 GET 방식일 때 인코딩/디코딩 처리를 했기 때문에 value값에 searchValue를 넣을 수 있음 -->
