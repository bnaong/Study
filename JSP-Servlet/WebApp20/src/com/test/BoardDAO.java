...
public class BoardDAO
{
	...
	/*
	 * public int getDataCount() { int result = 0;
	 * 
	 * Statement stmt = null; ResultSet rs = null; String sql = "";
	 * 
	 * try { sql = "SELECT COUNT(*) AS COUNT FROM TBL_BOARD"; stmt =
	 * conn.createStatement(); rs = stmt.executeQuery(sql); if (rs.next()) { result
	 * = rs.getInt(1); } rs.close(); stmt.close();
	 * 
	...
	// check~!!!
	// 검색 기능 추가 → searchKey : 제목 or 작성자 or 내용, searchValue : 입력값
	...
	// check~!!!
	// 검색 기능 추가 → searchKey : 제목 or 작성자 or 내용, searchValue : 입력값
	public List<BoardDTO> getLists(String searchKey, String searchValue, int start, int end)
	{
		List<BoardDTO> result = new ArrayList<BoardDTO>();
	...
	public int updateData(BoardDTO dto)
	{
		...
		try
		{
			sql = "UPDATE TBL_BOARD SET NAME=?, PWD=?, EMAIL=?"
