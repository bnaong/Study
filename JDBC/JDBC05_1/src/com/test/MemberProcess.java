...
public class MemberProcess
{
	...
	public void memberInsert()
	{
		...
		try
		{
			...
			for (String city : citys)
				cityStr.append(city + "/");
			// "강원/경북/경남/경북/부산/서울/인천/전남/전북/제주/충남/충북/"
			...
			지역(강원/경북/경남/경북/부산/서울/인천/전남/전북/제주/충남/충북/) : 경기
			전화번호 : 010-1111-1111
			...
			if (memList.size() > 0)
			{
				for (MemberDTO dto : memList)
				{
					System.out.printf("%5d %4s %14s %10s %4s %12s %4s %3s %8d %8d %8d\n"
							...
							, dto.getIbsadate(), dto.getCityName()
							, dto.getTel(), dto.getBuseoName()
							, dto.getJikwiName(), dto.getBasicPay()
	...
	public void memberUpdate()
	{
		...
		try
		{
			...
			if (memList.size() > 0)
			{
				...
				// 사용자에게 보여지는 화면 처리(- 입력시 수정을 하지않고 넘어감)
				/*
				직원 정보 수정 -----------------------------------------------------------------------------
				기존 이름 : 채다선
				수정 이름 : -
				
				기존 주민번호(yymmdd-nnnnnnn) : 961023-2234567
				수정 주민번호(yymmdd-nnnnnnn) : -
				
				기존 입사일(yyyy-mm-dd) : 2019-06-08
				수정 입사일(yyyy-mm-dd) : 2020-11-24
				
				기존 지역 : 경기
				수정 지역(강원/경북/경남/경북/부산/서울/인천/전남/전북/제주/충남/충북/) : -
				...
				기존 기본급 : 2500000
				수정 기본급(최소 1800000원 이상) : << 이거 바뀌어야함 입력받은대로
				...
				*/
				...
				String mIbasaDate = mMember.getIbsadate();
				...
				System.out.println("기존 주민번호(yymmdd-nnnnnnn) : " + mSsn);
				...
				System.out.println("기존 입사일(yyyy-mm-dd) : " + mIbasaDate);
				...
				// 새로 입력받은 (수정한) 내용을 통해 ScoreDTO 객체 구성
	...
	public void memberDelete()
	{
		...
		try
		{
			...
			// 사용자가 입력한 직원의 사원번호에 대한 유효성 겁사 코드 삽입 가능~!!!
