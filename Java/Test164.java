...
○ Enumeration 과 Iterator 인터페이스
   ...
   Collection Framework 에는 (데이터를 적재하고 일관성있게 접근하는 기능을 하기 위해 만든)
   ...
   - java.util.Enumeration<E> 인터페이스
	...
	- java.util.Iterater<E> 인터페이스
	 : Enumeration 과의 차이점은 단지 『remove()』 메소드가 추가된 것 뿐인다.
	   ...
	   (보완하기 위해서 나왔지만 Enumeration만 써야하는 collection도 있고, Iterator만 써야하는 collection도
	   있기 때문에 호환성의 문제로 둘 다 사용할 줄 알아야한다. 기능적으로 Enumeration을 썼을 때는
	   몇단계를 거쳐야 하지만 Iterator 메소드를 사용하면 바로 실행 가능한 경우 등이 있다. 
*/
