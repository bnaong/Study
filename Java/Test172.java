...
○ 자바의 기본 입출력 개요
	...
	- 자바에서의 모든 데이터의 입출력은 『스트림(stream)』이라는 개념에 의해
	  ...
	  "흐르는 물"(어디선가 어디로 흘러가는 물줄기)의 개념이며,
	  이는 연속된 일련의 데이터를 일컫는다.
	  (동시에 반대방향으로 흐르는 것은 불가능)

	- 데이터 입출력 시 모든 데이터를 형태와 관계없이 일련의 흐름으로
	  전송을 하는 것이 스트림 입출력 모델의 기본 개념이다.

	- 기본적으로 스트림(stream)에는
	  바이트 기반 스트림과 문자 기반 스트림가 있다.
	  (바이트기반 입력스트림, 문자기반 입력스트림, 바이트기반 출력스트림, 문자기반 출력스트림 4가지)


○ 바이트 스트림(byte-stream) 입출력

	- InputStream 클래스(입력되는 바이트스트림)의 OutputStream 클래스(출력되는 바이트스트림), 그리고 이들의 하위 클래스를
...
○ 문자 스트림(character-stream) 입출력
	...
	- Reader 클래스(입력되는 문자스트림)와 Writer 클래스(출력되는 문자스트림),
	  그리고 이들의 하위 클래스에 의해
...
※ 스트림(stream)의 특징
	...
	- 스트림은 단방향이다.
	  ...
	  따라서 읽기 쓰기가 필요하다면 읽는 스트림(읽는 물줄기)과 쓰는 스트림(쓰는 물줄기)을
	  하나씩 열어서 사용해야 한다.

	- 스트림은 지연될 수 있다.
	  스트림에 넣어진 데이터는 처리되기 전에는 스트림에 사용하는 스레드는
	  멈춰 있는 지연(blocking) 상태에 빠진다.
...
public class Test172
{
	public static void main(String[] args) throws IOException
	{
		...
		while ((data = System.in.read()) != -1)				// Ctrl+z ==> null => -1
		{
			ch = (char)data;
