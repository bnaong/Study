...
○ 객체 직렬화의 장점
	...
	객체 자체를 네트워크를 통해 손쉽게 교환(전송 및 수신)할 수 있게 된다.
	...
	또한, Bean 은 설계시 상태에 대한 정보를 저장 할 때,
...
○ Serialization 인터페이스
	...
	JVM 에 알려주는 역할을 수행한다.(Implement만 하면 객체직렬화가
	필요하다는 것을 전달함)
	...
	형식)
	1.
	public class 클래스명 implements Serialzable
	{
		...
	}
...
public class Test177
{
	...
	{
		// getProperty(); -> 시스템으로부터 Property(속성)을 문자열형으로 얻어오는 메소드
		...
		// 테스트(확인)
		//System.out.println(f0.getParentFile().exists());		// 경로가 존재하는가?
		//--==>> false											// 존재하지 않음
		// test.ser 파일이 만들어지게 될 디렉토리 경로가 구성되어 있지 않다.

		// ※ 『C:\JavaStudy』 경로에 『data』 디렉토리 생성~!!!

		// 생성 이후 다시 테스트(확인)
		//System.out.println(f0.getParentFile().exists());
		//--==>> true
		// test.ser 파일이 만들어지게 될 디렉토리 경로가 구성되어 있다.
